...
import { invokeTool, type ToolPartBuffer, type ToolTurn, type ToolTurnInput } from "./toolkit.ts";
...
export type CodexToolRunInput = ToolTurnInput & {
...
export const runTurnWithTools = (
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      yield* Stream.fromQueue(outbound).pipe(Stream.run(process.stdin), Effect.forkScoped);

      // Drain stderr: an unread pipe fills and stalls the app-server, and its
      // tail is the only diagnostic when it exits before completing the turn.
      let stderrTail = "";
      yield* Stream.runForEach(process.stderr.pipe(Stream.decodeText()), (chunk) =>
        Effect.sync(() => (stderrTail = (stderrTail + chunk).slice(-4096))),
      ...
      yield* Stream.runForEach(
        ...
        Effect.ensuring(Effect.suspend(() => Deferred.fail(turnDone, exitedEarly()))),
        Effect.forkScoped,
      );
          ...
          switch (msg.kind) {
            case "rpc-response": {
              ...
              if (msg.error !== undefined) {
                yield* Deferred.fail(
                  def.value,
                  fail(method, `codex app-server error: ${JSON.stringify(msg.error)}`),
                );
              } else {
